...
$postsAdm  = selectAllFromPostsWithhUser('posts', 'users');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_post'])){
    ...
    $title = trim($_POST['title']); //trim() - убирает лишние пробелы
    ...
    if($title === ''|| $content === '' || $topic === ''){
       array_push($errMsg,"Не все поля заполнены !");
        ...
        array_push($errMsg, "Название статьи должно быть больше 7-ми символов ");
    }else{
            $post = [
                'id_user' => $_SESSION['id'],
                'title' => $title,
